...
// Compactador e Descompactador de Arquivos com a codificação de Huffman

// Nó da árvore, que armazena o caracter, sua frequência e os nós direito e esquerdo
...
// Nó da lista, que armazena um nó da árvore e o próximo nó da lista
...
// Lista que tem um ponteiro para o primeiro nó e a quantidade de nós
...
// Novo nó da lista, que devolve um ponteiro com esse nó e recebe como parâmetro um nó de árvore
...
// Novo nó da árvore, que devolve um ponteiro para esse nó e recebe como parâmetro o caracter,
// sua frequência e os nós esquerdo e direito
...
// Obtém o número de vezes que o mesmo caracter apareceu e armazena esse valor na posição respectiva do vetor
...
// Insere um novo nó na lista, que ambos foram passados como parâmetros
...
// Remove o primeiro nó da lista e retorna um ponteiro para um nó da árvore
...
// Gera a árvore de Huffman, primeiro criando a lista com todos os caracteres, através da lista dos caracteres, que é passada como parâmetro e depois
// formando uma árvore, juntando os nós da lista e formando uma árvore (a de Huffman), usando o método diminuiLista() fazendo com que só sobre um nó
// na lista, que é a própria árvore de Huffman
...
// Começando na raíz, forma-se um caminho de 0s (esquerda), 1s (direita) e/ou nulo de acordo com caminho da raiz da árvore até uma folha, que representa
// cada caracter do arquivo; isso feito através de recursão
...
// Função recursiva que percorre a árvore e desaloca o endereço de memória de cada nó
...
// Método que pega 1 bit de 1 byte
...
// Método Main que contém um cabeçalho e puxa a função iniciar() e retorna 0
int main()
{
    printf("=-=-=-=-=-=-=-=-=-=-=\n");
    printf("     Compactador     \n");
...
// Método que inicia com um menu de operações para o usuário com switch, que vai compactar um arquivo, descompatá-lo, sair do programa ou então enviará
// um aviso de erro
int iniciar()
{
    printf("Operacao: \n\n");
